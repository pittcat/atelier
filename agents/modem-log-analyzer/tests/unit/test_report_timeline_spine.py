...
def test_timeline_not_empty_when_device_anomaly_claimed():
    ...
    md = _render()
    timeline_section = _section(md, "失败时间线")
...
def test_scenario_section_short_no_triple_repeat():
    ...
    md = _render()
    scenario_section = _section(md, "推断的测试场景与基线")
...
def test_empty_suggested_actions_renders_without_failure():
    ...
    md = _render()
    actions_section = _section(md, "建议行动")
    # 允许「无额外建议」之类占位
    assert "## 建议行动" in md
